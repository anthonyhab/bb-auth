...
bool isTemplateUnlockLine(const QString& line, const QString& target) {
    ...
    if (!normalizedTarget.isEmpty() && normalized.contains("unlock") && normalized.contains(normalizedTarget)) {
        if (normalized.startsWith("authenticate to unlock")
            || normalized.startsWith("unlock")
            || normalized.startsWith("use your password to unlock")
...
QString buildUnlockDetails(const QJsonObject& context, const QString& target) {
    ...
    const QStringList candidates = {
        normalizeDetailText(context.value("description").toString()),
        normalizeDetailText(context.value("message").toString()),
...
FallbackWindow::FallbackWindow(FallbackClient* client, QWidget* parent)
    : QWidget(parent)
    , m_client(client) {

    setWindowTitle("Authentication Required");
    setObjectName("NoctaliaAuthFallback");
    ...
    m_contentWidget->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Maximum);
    m_contentWidget->setMaximumWidth(640);
    ...
    m_contextToggleButton->setStyleSheet("QPushButton { color: #8fb59a; border: none; padding: 0; text-align: left; } QPushButton:hover { color: #a6c8af; text-decoration: underline; }");
    ...
    layout->addWidget(m_requestorLabel);
    layout->addWidget(m_contextLabel);
    layout->addWidget(m_contextToggleButton, 0, Qt::AlignLeft);
...
void FallbackWindow::ensureContentFits() {
    ...
    const int baselineHeight = m_baseHeight;

    const int targetWidth = qBound(m_minWidth, qMax(baselineWidth, requiredContentWidth + left + right), 680);
    const int targetHeight = qBound(m_minHeight, qMax(baselineHeight, requiredContentHeight + top + bottom + 8), 640);
...
FallbackWindow::PromptDisplayModel FallbackWindow::buildDisplayModel(const QJsonObject& event) const {
    ...
    if (!requestorName.isEmpty()) {
        const bool duplicateUnlockRequestor = (model.intent == PromptIntent::Unlock)
            && (requestorName.compare(unlockTarget, Qt::CaseInsensitive) == 0);
